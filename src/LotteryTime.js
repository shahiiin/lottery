  ...
  return (
    <div>
      <Link to={`/KashName/${props.id}`}>
        <button className="kashname-btn">

          <FaUserPlus style={{width: '1.5rem', height: '1.5rem'}} />
        </button>
      </Link>
    </div>
  );
...
const Menu2 = props => {
  return (
    <div>
      <Link to={`/FriendKash/${props.id}`}>
        <button className="friendkash-btn">
...
const Menu3 = props => {
  return (
    <div>
      <Link to={`/MainBank/${props.id}`}>
        <button className="mainbank-btn3">
...
const Menu4 = props => {
  return (
    <div>
      <Link to={`/CounterTicket/${props.id}`}>
        <button className="counterticket-btn">خرید بلیط</button>
      </Link>
    </div>
  );
};
  ...
  render () {
    return (
        ...
        <div className="lottery-body" style={{textAlign: 'center'}}>
          {this.state.users.map (item => (
            ...
            >
              ...
              <div>
                <h2 className="Lottery-header-2">مبلغ جایزه</h2>
              </div>
              <div>

                <p className="Lottery-param"> {item.price}</p>
              </div>
              <Card
                hoverable
                style={{width: 210, textAlign: 'center', marginLeft: 21}}
              >
                <div>
                  <img
      ...
      "
                    style={{width: 25, height: 20, borderRadius: 50}}
                  />
                  <p className="Lottery-content-param">تعداد شرکت کنندگان</p>
                </div>
                <div>

                  <p
                    className="Lottery-content-param-2"
                    style={{direction: 'rtl'}}
                  >
                    نفر
                  </p>

                  <p className="Lottery-content-param-3">مشاهده شرکت کنندگان</p>
                  <Menu2 />
                  <p className="Lottery-content-param-4"> {item.created_at}</p>
                </div>
                <div className="time">
                  <KashTime />
                </div>
              ...
              >
                <div>
                  <p className="Lottery-content-param-5">قیمت هر بلیط</p>
                  <p
                    className="Lottery-content-param-6"
                    style={{direction: 'rtl'}}
                  >
                    {item.price}
                  </p>
...
export default withRouter (LotteryTime);
